...
const PLAYER_INVENTER = ['Player1', 'Player2', 'Player3']; // Remplacez par vos joueurs fictifs
...
export class AppController {
  ...
  async getRanking(): Promise<any> {
    return await this.CacheRankingService.getRankingData();
  }
  ...
  async rankingEvent(@Res() res: Response): Promise<void> {
    ...
    const intervalId = setInterval(async () => { 
      const players = await this.CacheRankingService.getRankingData();
      const randomPlayer = players[Math.floor(Math.random() * players.length)];
      const newRank = Math.floor(Math.random() * 2500);

      // Update player's rank in the database
      await this.CacheRankingService.updateRank(randomPlayer.name, newRank);
  ...
  async postPlayer(@Res() res: Response, @Body() body: { id: string }): Promise<void> {
    const { id } = body;
    const averageRank = await this.CacheRankingService.getRankingMoyen();
    await this.playerService.addPlayer(id, averageRank);
    res.status(200).send(id);
  }

  @Post('/post/match')
  async postMatch(@Res() res: Response, @Body() body: { adversaire1: string, adversaire2: string, winner: string | null, draw: boolean }): Promise<void> {
